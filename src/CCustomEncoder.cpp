...
void CustomEncoder::encode_and_save(const std::string &outName, int blockSize)
{
    std::ofstream out(outName, std::ios::out | std::ios::binary);
    if (!out) throw std::runtime_error("ERR: Failed to save image (" + outName + ')');

    std::vector<int> selectedFilters = blockFilters::select_filters(m_width, m_height, m_channels, m_data, blockSize);
    std::vector<byte_t> filteredData = blockFilters::apply_filters(m_width, m_height, m_channels, m_data, blockSize, selectedFilters);
    ...
    uint8_t b = static_cast<uint8_t>(blockSize);
